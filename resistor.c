...
void hitung_3gelang()
{
    ...
    for (N=1; N<=3; N++)
    {
        printf(" Masukkan warna GELANG ke %d : ", N);
        scanf("%s", WARNA);
...
void hitung_4gelang()
{
    ...
    N=((G1*10)+G2)*G3;
    printf("\nNilai resistor %d ohm dengan toleransi %d%c", N, G4, p);
}
...
void hitung_5gelang()
{
    ...
    printf("\nNilai resistor %d ohm dengan toleransi %d%c", N, G5, p);
}
...
void hitung_6gelang()
{
    ...
    N=((((G1*100)+(G2*10))+G3)*G4);
    printf("\nNilai resistor %d ohm dengan toleransi %d%c %d ppm/K", N,G5,p,G6);
}
